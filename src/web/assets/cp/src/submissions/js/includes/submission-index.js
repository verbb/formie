...
Craft.Formie.SubmissionTableView = Craft.TableElementIndexView.extend({
    ...
    groupAndFillData(origin) {
        ...
        for (const [dateStr, value] of dataArray) {
            const key = formatDate(new Date(dateStr));
        ...
        return {
            data: Object.entries(results).map(([date, value]) => { return [date, value]; }),
            group: grouping,
        };
    },
    ...
    loadReport() {
        ...
        $elements.each((index, item) => {
            const dateCreated = $(item).data('date-created');
        ...
        const chartDataTable = new Craft.charts.DataTable(dataTable);
