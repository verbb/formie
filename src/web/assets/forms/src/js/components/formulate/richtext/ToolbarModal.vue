<template>
    <vue-final-modal
        class="fui-rt-modal-wrap"
        v-bind="$attrs"
        classes="fui-rt-modal-container"
        content-class="fui-rt-modal-content"
        overlay-class="fui-rt-modal-overlay"
        :lock-scroll="false"
        v-on="$listeners"
        @opened="opened"
    >
        <div class="fui-rt-modal-header">
            <slot name="title"></slot>
            <div class="fui-rt-modal-close" @click.prevent="$emit('input', false)"></div>
        </div>
        
        <div class="fui-rt-modal-body">
            <slot></slot>
        </div>

        <div class="fui-rt-modal-footer footer">
            <div class="spinner hidden"></div>

            <div class="buttons right">
                <div role="button" class="btn" tabindex="0" @click.prevent="$emit('cancel')">
                    {{ cancelButton | t('formie') }}
                </div>
                
                <div role="button" class="btn submit" @click.prevent="$emit('confirm')">
                    {{ confirmButton | t('formie') }}
                </div>
            </div>
        </div>
    </vue-final-modal>
...
.fui-rt-modal-wrap,
.fui-rt-modal-overlay {
    position: fixed !important;
}

.fui-rt-modal-container {
    display: flex;
    justify-content: center;
    align-items: center;
}

.fui-rt-modal-content {
    width: 90vw;
    height: 90vh;
    max-width: 650px;
    max-height: 450px;
