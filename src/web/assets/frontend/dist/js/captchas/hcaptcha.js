...
/* harmony import */ var _inc_hcaptcha__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./inc/hcaptcha */ "./src/js/captchas/inc/hcaptcha.js");
...
var FormieHcaptcha = /*#__PURE__*/function (_FormieCaptchaProvide) {
  ...
  var _super = _createSuper(FormieHcaptcha);
  ...
  function FormieHcaptcha() {
    ...
    _this.hCaptchaScriptId = 'FORMIE_HCAPTCHA_SCRIPT'; // Fetch and attach the script only once - this is in case there are multiple forms on the page.
    // They all go to a single callback which resolves its loaded state

    if (!document.getElementById(_this.hCaptchaScriptId)) {
      var $script = document.createElement('script');
      $script.id = _this.hCaptchaScriptId;
      $script.src = "https://js.hcaptcha.com/1/api.js?onload=formieHcaptchaOnLoadCallback&recaptchacompat=off&render=explicit&hl=".concat(_this.language);
    ...
    _inc_hcaptcha__WEBPACK_IMPORTED_MODULE_1__.hcaptcha.checkRecaptchaLoad(); // We can have multiple captchas per form, so store them and render only when we need

    _this.$placeholders = _this.$form.querySelectorAll('[data-hcaptcha-placeholder]');
    ...
    _this.form.addEventListener(_this.$form, (0,_utils_utils__WEBPACK_IMPORTED_MODULE_2__.eventKey)('onFormieCaptchaValidate', 'Hcaptcha'), _this.onValidate.bind(_assertThisInitialized(_this)));

    _this.form.addEventListener(_this.$form, (0,_utils_utils__WEBPACK_IMPORTED_MODULE_2__.eventKey)('onAfterFormieSubmit', 'Hcaptcha'), _this.onAfterSubmit.bind(_assertThisInitialized(_this)));
  ...
  _createClass(FormieHcaptcha, [{
    ...
    value: function renderCaptcha() {
      ...
      _inc_hcaptcha__WEBPACK_IMPORTED_MODULE_1__.hcaptcha.render(this.createInput(), {
    ...
    value: function onValidate(e) {
      ...
      this.submitHandler = e.detail.submitHandler; // Check if the captcha has already been solved (someone clicking on the tick), otherwise the captcha triggeres twice

      if (this.token) {
        this.onVerify(this.token);
      } else {
        // Trigger hCaptcha - or check
        _inc_hcaptcha__WEBPACK_IMPORTED_MODULE_1__.hcaptcha.execute(this.hcaptchaId);
      }
    }
  }, {
    key: "onVerify",
    value: function onVerify(token) {
      ...
      this.token = token; // Submit the form - we've hijacked it up until now
    ...
    value: function onExpired() {
      ...
      _inc_hcaptcha__WEBPACK_IMPORTED_MODULE_1__.hcaptcha.reset(this.hcaptchaId);
      this.token = null;
    }
  }, {
