...
var FormieJSCaptcha = /*#__PURE__*/function (_FormieCaptchaProvide) {
  _inherits(FormieJSCaptcha, _FormieCaptchaProvide);
  function FormieJSCaptcha() {
    var _this;
    var settings = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    _classCallCheck(this, FormieJSCaptcha);
    _this = _callSuper(this, FormieJSCaptcha, [settings]);
    _this.$form = settings.$form;
    _this.form = _this.$form.form;
    _this.sessionKey = settings.sessionKey;
    _this.$placeholder = _this.$form.querySelector('[data-jscaptcha-placeholder]');
    ...
    _this.value = window["Formie".concat(_this.sessionKey)];
    if (!_this.value) {
      console.error("Unable to find JavaScript Captcha value for Formie".concat(_this.sessionKey));
      return _possibleConstructorReturn(_this);
    }
    _this.createInput();
    return _this;
  }
  _createClass(FormieJSCaptcha, [{
    key: "createInput",
    value: function createInput() {
      // We need to handle re-initializing, so always empty the placeholder to start fresh to prevent duplicate captchas
      this.$placeholder.innerHTML = '';
