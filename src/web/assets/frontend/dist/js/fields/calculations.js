  ...
  _createClass(FormieCalculations, [{
    ...
    value: function initCalculations() {
      ...
      Object.keys(this.variables).forEach(function (variableKey) {
        ...
        var $targets = _this.$form.querySelectorAll("[name=\"".concat(variable.name, "\"]")); // Special handling for Repeater/Groups that have `new1` in their name but for page reload forms
        ...
        if ((!$targets || !$targets.length) && variable.name.includes('[new1]')) {
          ...
          var regexString = variable.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/new1/g, '\\d+'); // Find all targets via Regex.
      ...
      this.form.addEventListener(this.$field, (0,_utils_utils__WEBPACK_IMPORTED_MODULE_0__.eventKey)('onFormieEvaluateCalculations'), this.evaluateCalculations.bind(this)); // Also - trigger the event right now to evaluate immediately. Namely if we need to hide
    ...
    value: function evaluateCalculations(e) {
      ...
      var variables = {}; // For each variable, grab the value
      ...
      Object.keys(this.fieldsStore).forEach(function (variableKey) {
        var _this2$fieldsStore$va = _this2.fieldsStore[variableKey],
            ...
            type = _this2$fieldsStore$va.type; // Set a sane default

        variables[variableKey] = ''; // We pass target DOM elements as a NodeList, but in almost all cases,
      ...
      variables = this.formatVariables(variables); // Allow events to modify the data before evaluation
      ...
      $field.dispatchEvent(beforeEvaluateEvent); // Events can modify the formula and variables
      // eslint-disable-next-line

      formula = beforeEvaluateEvent.detail.formula; // eslint-disable-next-line

      variables = beforeEvaluateEvent.detail.variables; // Prevent evaluating empty data
      ...
      try {
        var result = this.expressionLanguage.evaluate(formula, variables); // Format the result, if required

        result = this.formatValue(result); // Allow events to modify the data after evaluation
        ...
        $field.dispatchEvent(afterEvaluateEvent); // Events can modify the result
        // eslint-disable-next-line

        result = afterEvaluateEvent.detail.result; // Handle null-like results. If they're `NaN`, `false` set as empty, but `0` is valid
        ...
        console.error(ex); // Always reset in the event of an error
