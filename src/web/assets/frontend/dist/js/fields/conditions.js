...
var FormieConditions = /*#__PURE__*/function () {
  function FormieConditions() {
    ...
    this.$form.querySelectorAll('[data-field-type="repeater"]').forEach(function ($field) {
      $field.addEventListener('initRow', function (e) {
        _this.initFieldConditions(e.detail.$row);
      });
    });
  }
  _createClass(FormieConditions, [{
    ...
    value: function initFieldConditions($container) {
      ...
      $container.querySelectorAll('[data-field-conditions]').forEach(function ($field) {
        var conditionSettings = _this2.parseJsonConditions($field);
        ...
        conditionSettings.conditions.forEach(function (condition) {
          ...
          var $targets = $container.querySelectorAll("[name=\"".concat(condition.field, "\"]"));

          // Check if we're dealing with multiple fields, like checkboxes. This overrides the above
          var $multiFields = $container.querySelectorAll("[name=\"".concat(condition.field, "[]\"]"));
          ...
          $targets.forEach(function ($target) {
            ...
            var eventType = _this2.getEventType($target);
            ...
            _this2.form.addEventListener($target, (0,_utils_utils__WEBPACK_IMPORTED_MODULE_0__.eventKey)(eventType), function () {
        ...
        _this2.conditionsStore.set($field, {
      ...
      conditions.forEach(function (condition, i) {
        ...
        $targets.forEach(function ($target) {
          ...
          if ($target.getAttribute('data-fui-input-type') === 'agree') {
            ...
            result = _this3.testCondition(logic, value == '0' ? false : true, $target.checked);
            results[resultKey].push(result);
          } else if (inputType === 'checkbox' || inputType === 'radio') {
            // Handle (multi) checkboxes and radio, which are a bit of a pain
            result = _this3.testCondition(logic, value, $target.value) && $target.checked;
            results[resultKey].push(result);
          } else if (tagName === 'select' && $target.hasAttribute('multiple')) {
            // Handle multi-selects
            Array.from($target.options).forEach(function ($option) {
              result = _this3.testCondition(logic, value, $option.value) && $option.selected;
              results[resultKey].push(result);
            });
          } else {
            result = _this3.testCondition(logic, value, $target.value, testOptions);
            results[resultKey].push(result);
          }
        });
      });
