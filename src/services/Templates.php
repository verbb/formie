    ...
    private function _getObjectTokens(mixed $object): array
    {
        ...
        if ($object instanceof Submission) {
            ...
            foreach ($object->getFields() as $field) {
                ...
                if ($field->fieldKey && $field->fieldKey !== $handle) {
                    $tokens[$field->fieldKey] = $value;
                    $tokens["field.$field->fieldKey"] = $value;
                    $tokens["field:$field->fieldKey"] = $value;
                    $this->_addNestedTokens($tokens, $field->fieldKey, $fieldValue);
                    $this->_addNestedTokens($tokens, "field.$field->fieldKey", $fieldValue);
                    $this->_addNestedTokens($tokens, "field:$field->fieldKey", $fieldValue);
                }
            }
