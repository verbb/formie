...
class SubmissionStatuses extends Component
{
    ...
    use SubmissionStatusesDeprecations;
    ...
    {
        return Formie::$plugin->getFormGroupPolicy()->getSubmissionStatusesForForm($form);
    }
    ...
    public function getStatusById(int $id): ?SubmissionStatus
    {
        return $this->_statuses()->firstWhere('id', $id);
    }

    public function getStatusByHandle(string $handle): ?SubmissionStatus
    {
        return $this->_statuses()->firstWhere('handle', $handle, true);
    }

    public function getStatusByUid(string $uid): ?SubmissionStatus
    {
    ...
    public function reorderStatuses(array $statusIds): bool
    {
        ...
        $uidsByIds = Db::uidsByIds(Table::FORMIE_SUBMISSION_STATUSES, $statusIds);

        foreach ($statusIds as $statusOrder => $statusId) {
            if (!empty($uidsByIds[$statusId])) {
                $statusUid = $uidsByIds[$statusId];
                $projectConfig->set(self::CONFIG_SUBMISSION_STATUSES_KEY . '.' . $statusUid . '.sortOrder', $statusOrder + 1, 'Reorder statuses');
            }
        }
    ...
    public function saveStatus(SubmissionStatus $status, bool $runValidation = true): bool
    {
        ...
        if ($this->hasEventHandlers(self::EVENT_BEFORE_SAVE_STATUS)) {
            $this->trigger(self::EVENT_BEFORE_SAVE_STATUS, new SubmissionStatusEvent([
                'status' => $status,
                'isNew' => $isNewStatus,
            ]));
        }
        ...
        $configPath = self::CONFIG_SUBMISSION_STATUSES_KEY . '.' . $status->uid;
        Craft::$app->getProjectConfig()->set($configPath, $status->getConfig(), "Save the “{$status->handle}” status");

        if ($isNewStatus) {
            $status->id = Db::idByUid(Table::FORMIE_SUBMISSION_STATUSES, $status->uid);
        }

        return true;
    }

    public function handleChangedSubmissionStatus(ConfigEvent $event): void
    {
        ...
        if ($this->hasEventHandlers(self::EVENT_AFTER_SAVE_STATUS)) {
            $this->trigger(self::EVENT_AFTER_SAVE_STATUS, new SubmissionStatusEvent([
    ...
    public function deleteStatus(SubmissionStatus $status): bool
    {
        ...
        if ($this->hasEventHandlers(self::EVENT_BEFORE_DELETE_STATUS)) {
            $this->trigger(self::EVENT_BEFORE_DELETE_STATUS, new SubmissionStatusEvent([
                'status' => $status,
            ]));
        }

        Craft::$app->getProjectConfig()->remove(self::CONFIG_SUBMISSION_STATUSES_KEY . '.' . $status->uid, "Delete status “{$status->handle}”");

        return true;
    }

    public function handleDeletedSubmissionStatus(ConfigEvent $event): void
    {
        ...
        if ($this->hasEventHandlers(self::EVENT_BEFORE_APPLY_STATUS_DELETE)) {
            $this->trigger(self::EVENT_BEFORE_APPLY_STATUS_DELETE, new SubmissionStatusEvent([
                'status' => $status,
            ]));
        }
        ...
        if ($this->hasEventHandlers(self::EVENT_AFTER_DELETE_STATUS)) {
            $this->trigger(self::EVENT_AFTER_DELETE_STATUS, new SubmissionStatusEvent([
                'status' => $status,
            ]));
        }
    }
    ...
    private function _getStatusRecord(string $uid, bool $withTrashed = false): SubmissionStatusRecord
    {
        $query = $withTrashed ? SubmissionStatusRecord::findWithTrashed() : SubmissionStatusRecord::find();
        $query->andWhere(['uid' => $uid]);

        return $query->one() ?? new SubmissionStatusRecord();
    }
}
