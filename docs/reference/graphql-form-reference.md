# GraphQL Form Reference

Use this reference to look up form query arguments, field properties and generated settings types. Start with [Query Forms](/graphql/query-forms) to fetch a form, or [the submission walkthrough](/guides/frontend-headless/graphql-submission-flow-end-to-end) to build a working client. The available fields depend on the form layout and installed field providers.
...
The `settings` field exposes the form-level settings that commonly affect rendering and submission behaviour.
...
`borderColor` | `String` | The section border colour.

### Signature

Field | Type | Description
--- | --- | ---
`backgroundColor` | `String` | The signature pad background colour.
`penColor` | `String` | The signature pen colour.
