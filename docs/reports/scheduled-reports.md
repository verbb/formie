...
1. Create and save a [report](/reports/reports) with the filters and columns you want.
...
Test sends use the full current report filters and do **not** update `lastSentAt`. They send to the signed-in user who runs the test, not to the configured recipient list. This checks rendering and delivery to your own mailbox; verify the intended recipients separately and inspect the first scheduled delivery.
