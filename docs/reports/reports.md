...
All export formats write rows incrementally to disk, including Excel, JSON and XML. Formie records the matching submission IDs before exporting so new submissions cannot shift rows between batches. Field values are read as each batch is processed; submissions that are deleted or no longer match the filters are omitted. Scheduled reports still attach small exports to the summary email; when an export exceeds the **Maximum Email Attachment Size** (**Settings → Notifications**), the email includes a signed download link instead.

If a queue worker stops during an export, retry the failed job in Craft’s queue. The retry rebuilds the unfinished file and removes abandoned spreadsheet parts. Completed exports retain their existing download links when a job is repeated. If the backup email fails, the completed file remains available in the control panel.
