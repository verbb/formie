...
| `data-formie-form` | Form root hook for browser behaviour | Required |
| `data-formie-handle` | Form identity for browser loads and submits | Required |
| `data-formie-static-cache` | Static-cache behaviour hint | Preserve when present |
| `data-formie-submit-method` | Submit transport behaviour | Preserve when present |
| `data-formie-submit-action` | Submit action behaviour | Preserve when present |
| `data-formie-submit-action-form-hide` | Post-submit form hiding behaviour | Preserve when present |
...
| `data-formie-validation-on-focus` | Live-validation behaviour | Preserve when present |
| `data-formie-validation-on-submit` | Submit-time validation behaviour | Preserve when present |
