...
function B(e) {
  return Array.from(e.querySelectorAll(g)).reduce((o, a) => {
    const i = parseInt(a.getAttribute(y) || "", 10);
...
function p(e, o) {
  if (!e)
    return;
  const a = parseInt(e.getAttribute("data-formie-max-rows") || "", 10);
  e.disabled = a > 0 && o >= a;
}
function C(e, o) {
  const a = e.querySelector(H), i = e.querySelector(x), t = e.querySelector(_);
  ...
  const w = () => {
    e.querySelectorAll(O).forEach((r) => {
      if (!(r instanceof HTMLElement) || u.has(r))
        return;
      const n = (d) => {
        d.preventDefault();
        const l = r.closest(g);
      ...
      r.addEventListener("click", n), u.set(r, n);
    });
  }, T = async () => {
    if (o.static || !(t instanceof HTMLButtonElement) || !t.getAttribute("data-formie-table-add"))
      return;
    const n = t.getAttribute(k) || e.getAttribute(k), d = parseInt(t.getAttribute("data-formie-max-rows") || "", 10);
    if (d > 0 && c(e) >= d)
      return;
    const l = z(e, n);
    if (!l)
    ...
    m.setAttribute("data-formie-table-row", "true"), m.setAttribute(y, String(v - 1)), m.innerHTML = f, i.appendChild(m), await R(50), w(), p(t, c(e)), h(e, s, "append", {
      table: e,
      row: m
    });
  }, E = (r) => {
  ...
  return t instanceof HTMLButtonElement && !o.static && t.addEventListener("click", E), w(), p(t instanceof HTMLButtonElement ? t : null, c(e)), h(e, s, "init", {
    ...
    t instanceof HTMLButtonElement && t.removeEventListener("click", E), u.forEach((r, n) => {
...
const N = {
  ...
  setup: async (e) => {
    ...
    if (!(e.target instanceof HTMLElement))
      return;
    const a = e.target.matches(b) ? [e.target] : Array.from(e.target.querySelectorAll(b)).filter((t) => t instanceof HTMLElement), i = a.map((t) => C(t, o));
