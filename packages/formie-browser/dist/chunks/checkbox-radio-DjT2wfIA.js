import { r as A, d as E, a as p, e as M } from "./shared-BBFZJt2w.js";
import { h as L } from "./index-BYQHeX5S.js";
const u = "[data-formie-checkboxes-field-layout], [data-formie-radio-field-layout]", d = "minmaxOptions", h = "data-formie-checkbox-radio-max-disabled", g = "checkbox-radio", x = "checkbox-radio", f = L("fields", "checkbox-radio");
function m(t) {
  ...
  A(t, x, (r) => {
    r.addValidator(
      ...
      ({ field: o, getRule: e }) => {
        if (!o || !e(d))
          return !0;
        const n = Array.from(o.querySelectorAll('input[type="checkbox"]')).filter((l) => l instanceof HTMLInputElement && !m(l)).filter((l) => l.checked).length, { min: a, max: i } = b(e);
        return !(a !== null && n < a || i !== null && n > i);
      },
      ({ field: o, label: e, t: c, getRule: n }) => {
        const { min: a, max: i } = o ? b(n) : { min: null, max: null };
        return a !== null && i !== null ? c("{label} must select between {min} and {max}.", { label: e, min: a, max: i }) : a !== null ? c("{label} must select no less than {min}.", { label: e, min: a }) : i !== null ? c("{label} must select no greater than {max}.", { label: e, max: i }) : c("{label} has an invalid value.", { label: e });
      }
    );
  });
}
...
const H = {
  id: g,
  kind: "field",
  match: (t) => t.target instanceof HTMLElement && (t.target.matches(u) || !!t.target.querySelector(u)),
  setup: async (t) => {
    if (!(t.target instanceof HTMLElement))
      return;
    const r = t.target.matches(u) ? [t.target] : Array.from(t.target.querySelectorAll(u)).filter((e) => e instanceof HTMLElement);
