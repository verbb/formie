...
function pt(e, t) {
	...
	return q(e, t).forEach((e) => {
		let t = W(e.value), r = e.getAttribute(et)?.trim() || "";
		t && r && (n[t] = r);
	}), n;
}
function mt(e, t, n, r) {
	...
	q(e, n).forEach((e) => {
		...
		let r = document.createElement("input");
		r.type = "hidden", r.name = n, r.value = String(e), r.setAttribute($e, "true"), t && r.setAttribute(et, t), i.insertAdjacentElement("afterend", r), i = r;
	});
}
function ht(e, t, n) {
...
function gt(e, t, n, r = {}) {
	let i = new FormData(), a = dt(e), o = G(t);
	return a && i.append("handle", a), o && i.append("fieldHandle", o), [
		...
		let n = e?.querySelector(`input[name="${t}"]`);
		n instanceof HTMLInputElement && n.value.trim() && i.append(t, n.value.trim());
	}), n.forEach((e) => {
...
function vt(e, t, n) {
	let { track: r, bar: i, label: a } = X(e), o = Math.max(0, Math.min(st, Math.round(t)));
...
function $(e) {
	...
	mt(e.field, e.anchorInput, e.assetInputName, t), e.statusInput && (e.statusInput.value = t.length ? "uploaded" : ""), p(e.field, "file-upload", "uploaded-assets-sync", { assets: t.map(({ assetId: e, filename: t }) => ({
		assetId: e,
		filename: t
	})) });
}
...
function kt(e, t, n) {
	...
	let d = Ot(t, e), f = u instanceof HTMLInputElement ? u : null, m = Q(e), h = ft(e, m), g = Y(i, n.uploadEndpoint, "data-formie-file-upload-upload-endpoint", "/actions/formie/file-upload/upload"), _ = Y(i, n.deleteEndpoint, "data-formie-file-upload-delete-endpoint", "/actions/formie/file-upload/delete"), v = Y(i, n.hydrateEndpoint, "data-formie-file-upload-hydrate-endpoint", "/actions/formie/file-upload/hydrate"), y = n.limitFiles ?? W(i.getAttribute("data-formie-file-limit")), b = n.sizeLimit ?? parseFloat(i.getAttribute("data-formie-size-max-limit") || ""), x = n.sizeMinLimit ?? parseFloat(i.getAttribute("data-formie-size-min-limit") || ""), S = _t(n.accept ?? i.getAttribute("accept") ?? a.accept), C = {};
	...
	w.use(Ge, {
		...
		onBeforeRequest(e, t, n) {
			...
			!i?.id || !i.size || e.upload.addEventListener("progress", (e) => {
				...
				if (!t) return;
				let n = 0;
				e.lengthComputable && e.total > 0 ? n = Math.round(e.loaded / e.total * 100) : e.loaded > 0 && i.size && (n = Math.min(100, Math.round(e.loaded / i.size * 100))), yt(t.listItem, n, {
					loaded: e.loaded,
	...
	let E = {
		...
		uppy: w,
		files: []
	}, D = () => y ? Math.max(0, y - Tt(E)) : null, O = (e) => {
		...
		if (t.uppyFileId && w.removeFile(t.uppyFileId), t.assetId) {
			let n = new FormData(), r = ht(d, e, i);
			Object.entries(r).forEach(([e, t]) => {
				n.append(e, t);
			}), n.append("assetId", String(t.assetId)), t.uploadToken && n.append("uploadToken", t.uploadToken);
			try {
				await c(_, {
					method: "POST",
					body: n
		...
		let { listItem: i, removeButton: a, sortUpButton: o, sortDownButton: s } = wt(e, n);
		i.classList.add("is-complete"), l.append(i);
		let c = {
			assetId: t,
			uploadToken: r,
			filename: n,
		...
		try {
			...
			t.forEach((e) => {
				...
				let t = i.get(e);
				t && ne(e, t.filename || `Asset #${e}`, ut(t.uploadToken) || n[e] || null);
			}), $(E), k();
		} catch (e) {
	...
	w.on("upload-progress", (e, t) => {
		...
		let n = M(e?.id);
		if (!n) return;
		let r = n.listItem.querySelector("[data-formie-upload-manager-error]"), i = lt(t?.body) ? t.body : {}, a = W(i.assetId);
		if (!i.success || !a) {
			if (n.listItem.classList.add("is-error"), n.listItem.classList.remove("is-uploading", "is-upload-complete", "is-processing", "is-preparing"), r instanceof HTMLElement) {
				r.hidden = !1, r.textContent = bt(null, { message: "Upload failed." });
				let e = i.errors ? Object.values(i.errors).flat().find(Boolean) : null;
				e && (r.textContent = e);
			}
			return;
		}
		n.assetId = a, n.uploadToken = ut(i.uploadToken) || null, n.filename = i.filename || n.filename, St(n.listItem), $(E), k();
	}), w.on("upload-error", (e, t, n) => {
		let r = M(e?.id);
		if (!r) return;
		let i = r.listItem.querySelector("[data-formie-upload-manager-error]");
		r.listItem.classList.add("is-error"), r.listItem.classList.remove("is-uploading", "is-upload-complete", "is-processing", "is-preparing"), i instanceof HTMLElement && (i.hidden = !1, i.textContent = bt(n, t));
	}), w.on("file-added", (t) => {
		let n = D();
		if (n !== null && n <= 0) {
		...
		w.setFileMeta(t.id, ht(d, e, i));
		let { listItem: r, removeButton: a, sortUpButton: o, sortDownButton: s } = wt(e, t.name || "Upload");
		l.append(r);
		let c = {
			assetId: null,
			uploadToken: null,
	...
	let P = () => {
		E.files.forEach((e) => {
			e.listItem.remove();
		}), E.files = [], w.cancelAll(), mt(e, h, m, []);
	};
	return d?.addEventListener(tt, P), re(), () => {
		d?.removeEventListener(tt, P), w.destroy(), E.files = [];
	};
}
var At = {
	id: rt,
	...
	setup: async (e) => {
		...
		let a = (e) => {
			...
			e instanceof HTMLElement && e.hasAttribute("data-formie-field-handle") && a.add(e), e instanceof Element && (e.querySelectorAll("[data-formie-field-handle]").forEach((e) => {
				...
				r.has(e) || !e.querySelector(z) || (r.add(e), i.push(kt(e, n, t)));
			});
		};
		a(e.target);
		let o = (e) => {
		...
		return n?.addEventListener(nt, o), U.log("Module setup.", { count: i.length }), await e.emit("formie:module:upload-manager:init", { count: i.length }), { destroy: () => {
			n?.removeEventListener(nt, o), i.forEach((e) => {
				e();
			}), Dt(n), U.log("Module destroy."), e.emit("formie:module:upload-manager:destroy", {});
		} };
	}
};
//#endregion
export { At as uploadManagerModule };
