...
function m(e) {
	return { accessToken: e.querySelector("[data-formie-summary-token]")?.value?.trim() || null };
}
async function h(e, t, r) {
	if (!t.accessToken) throw Error("Summary field requires an access token.");
	let i = new FormData(e);
	return i.set("action", u), i.set("accessToken", t.accessToken), n(p(), {
		method: "POST",
		body: i,
		signal: r,
...
function g(e, n) {
	...
	if (!(i instanceof HTMLFormElement)) return f.warn("Missing form ancestor; skipping field."), () => {};
	let a = !1, s = !0, u = !1, p = 0, g = 0, _ = null, v = () => {
	...
	b(!!m(e).accessToken);
	let x = () => {
		!u || a && !s || (f.log("Queueing fetch."), S());
	}, S = r(async () => {
		let t = m(e);
		if (!v() || !t.accessToken) {
			f.warn("Missing state for fetch.", t), b(!1);
			return;
		}
		g += 1;
		let n = g, r = p;
		_?.abort(), _ = new AbortController(), b(!0);
		try {
			let c = await h(i, t, _.signal);
			if (n !== g) return;
			let u = y(), m = document.createElement("template");
			m.innerHTML = c.trim();
			let v = m.content.querySelector(l);
			u && v instanceof HTMLElement ? u.replaceWith(v) : u && (u.innerHTML = c), a = !0, s = p !== r, f.log("Fetch complete.", {
				isDirty: s,
				dirtyVersion: p,
				requestVersion: n
			}), o(e, d, "fetch-summary", {
		...
		let n = t?.target;
		n instanceof Node && e.contains(n) || (s = !0, p += 1, f.log("Marked dirty.", { dirtyVersion: p }));
	}, w = (e) => {
		C(e), x();
	}, T = () => {
		s = !0, f.log("Submit result received; refreshing."), x();
	}, E = () => {
		s = !0, f.log("Page navigation received; refreshing."), x();
	}, D = new IntersectionObserver((t) => {
		u = !!t[0]?.isIntersecting, u && (f.log("Field became visible."), o(e, d, "field-visible", { summary: e }), x());
	}, {
		root: i,
		rootMargin: "50px"
	});
	return D.observe(e), i.addEventListener("input", w), i.addEventListener("change", w), n.addEventListener("formie:page:navigate:after", E), n.addEventListener("formie:submit:result", T), () => {
		_?.abort(), D.disconnect(), i.removeEventListener("input", w), i.removeEventListener("change", w), n.removeEventListener("formie:page:navigate:after", E), n.removeEventListener("formie:submit:result", T), f.log("Field destroyed.");
	};
}
var _ = {
	id: d,
	...
	setup: async (e) => {
		let t = a(e).map((t) => g(t, e.root));
		return f.log("Module setup.", { fieldCount: t.length }), await e.emit("formie:module:summary:init", { count: t.length }), { destroy: () => {
			t.forEach((e) => {
				e();
			}), f.log("Module destroy.", { fieldCount: t.length }), e.emit("formie:module:summary:destroy", {});
		} };
	}
};
//#endregion
export { _ as summaryModule };
