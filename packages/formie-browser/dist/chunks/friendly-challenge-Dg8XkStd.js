...
function getDoneHTML(fieldName, l, solution, data) {
	const timeData = `${data.t.toFixed(0)}s (${(data.h / data.t * .001).toFixed(0)}K/s)${data.solver === SOLVER_TYPE_JS ? " JS Fallback" : ""}`;
...
function injectStyle(styleNonce = null) {
	if (!document.querySelector("#frc-style")) {
		...
		if (styleNonce) styleSheet.setAttribute("nonce", styleNonce);
...
function updateProgressBar(element, data) {
	...
	if (p) {
		...
		p.title = data.i + 1 + "/" + data.n + " (" + (data.h / data.t * .001).toFixed(0) + "K/s)";
	}
}
/**
...
*/
function isHeadless() {
	return ua.indexOf("headless") !== -1 || nav.appVersion.indexOf("Headless") !== -1 || ua.indexOf("bot") !== -1 || ua.indexOf("crawl") !== -1 || nav.webdriver === true || !nav.language || nav.languages !== void 0 && !nav.languages.length;
}
/**
...
async function getPuzzle(urlsSeparatedByComma, siteKey, lang) {
	...
	for (let i = 0; i < urls.length; i++) try {
		...
		if (response.ok) return (await response.json()).data.puzzle;
		else {
			let json;
			try {
				json = await response.json();
			} catch (e) {}
			if (json && json.errors && json.errors[0] === "endpoint_not_enabled") throw Error(`Endpoint not allowed (${response.status})`);
			if (i === urls.length - 1) throw Error(`Response status ${response.status} ${response.statusText} ${json ? json.errors : ""}`);
		}
	} catch (e) {
		console.error("[FRC Fetch]:", e);
		const err = /* @__PURE__ */ new Error(`${lang.text_fetch_error} <a class="frc-err-url" href="${urls[i]}">${urls[i]}</a>`);
		err.rawError = e;
		throw err;
	}
...
var URL;
if (typeof window !== "undefined") URL = window.URL || window.webkitURL;
var WorkerGroup = class {
		...
		for (let i = 0; i < this.workers.length; i++) {
			...
			this.workers[i].onmessage = (e) => {
				...
				if (data.type === "ready") {
					...
					if (data.puzzleNumber !== this.puzzleNumber) return;
				...
				} else if (data.type === "error") this.errorCallback(data);
			};
		}
	}
	setupSolver(forceJS = false) {
		...
		for (let i = 0; i < this.workers.length; i++) this.workers[i].postMessage(msg);
	}
	async start(puzzle) {
...
var WidgetInstance = class {
	constructor(element, options = {}) {
		...
		*/
		...
		if (!this.opts.skipStyleInjection) injectStyle(this.opts.styleNonce);
	...
	init(forceStart) {
		...
		if (forceStart) this.start();
			...
			if (form) executeOnceOnFocusInEvent(form, () => this.start());
			else console.log("FriendlyCaptcha div seems not to be contained in a form, autostart will not work");
		}
	}
	/**
	...
	*/
	loadLanguage(lang) {
		if (lang !== void 0) this.opts.language = lang;
		else if (this.e.dataset["lang"]) this.opts.language = this.e.dataset["lang"];
		if (typeof this.opts.language === "string") {
			...
			this.lang = l;
		} else this.lang = Object.assign(Object.assign({}, localizations.en), this.opts.language);
	}
	/**
	...
	*/
	...
	initWorkerGroup() {
		...
		this.workerGroup.doneCallback = (data) => {
			...
			if (callback) window[callback](solutionPayload);
		};
	...
	async start() {
		...
		try {
			...
			if (callback) window[callback](this);
			return;
		}
