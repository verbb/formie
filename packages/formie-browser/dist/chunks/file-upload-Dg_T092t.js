import { r as V, a as K, d as O } from "./shared-BDEKVuB5.js";
import { e as q } from "./styles-C3aqgtek.js";
import { l as B, m as j, o as W, p as Y } from "./index-CZtn5KAB.js";
const $ = "@layer formie-theme{.formie-file-input{padding:var(--formie-space-1);line-height:var(--formie-line-height-base);cursor:pointer}.formie-file-input::file-selector-button,.formie-file-input::-webkit-file-upload-button{appearance:none;-webkit-appearance:none;margin-inline-end:var(--formie-space-2);padding:calc(var(--formie-control-padding-y) - 1px) var(--formie-space-2);min-height:calc(var(--formie-control-height) - (var(--formie-space-1) * 2));border:var(--formie-border-width) solid var(--formie-color-border-control);border-radius:calc(var(--formie-radius-sm) - 1px);background:var(--formie-color-surface-subtle);color:var(--formie-color-heading);font-weight:var(--formie-font-weight-normal);font-size:var(--formie-font-size-xs);line-height:1.1;white-space:nowrap;cursor:pointer;transition:border-color .15s ease,background-color .15s ease,color .15s ease,box-shadow .15s ease}.formie-file-input:hover::file-selector-button,.formie-file-input:hover::-webkit-file-upload-button{border-color:color-mix(in srgb,var(--formie-color-border-control) 70%,var(--formie-color-heading) 30%);background:var(--formie-color-surface-muted)}.formie-file-input:focus{outline:0}.formie-file-input:focus-visible::file-selector-button,.formie-file-input:focus-visible::-webkit-file-upload-button{border-color:var(--formie-color-focus-ring)}.formie-field-has-error .formie-file-input::file-selector-button,.formie-field-has-error .formie-file-input::-webkit-file-upload-button{border-color:var(--formie-color-danger)}.formie-file-summary{padding:var(--formie-file-summary-padding);gap:var(--formie-gap-file-summary);border:var(--formie-border-width) solid var(--formie-color-border-control);border-radius:var(--formie-radius-sm)}.formie-file-summary-container{margin:0;padding-left:var(--formie-list-indent)}}", _ = 'input[type="file"][data-formie-file-input]', F = j("uploaded"), L = W("reset"), J = "data-formie-file-upload-key", G = "data-formie-file-upload-hydrate-endpoint", b = "data-formie-file-upload-anchor", Q = "data-formie-file-upload-asset-id", X = [
  "fileLimit",
  "fileSizeMinLimit",
  "fileSizeMaxLimit"
...
function C(e, t) {
  const r = k(t);
  return x(e).filter((n) => n.name === r && n.value.trim() !== "");
}
function te(e, t) {
  const r = D(t), n = x(e).find((s) => s.hasAttribute(b) || s.name === r && s.value === "");
  if (n)
    return n.setAttribute(b, "true"), n.name = r, n.value = "", n;
  const i = document.createElement("input");
  return i.type = "hidden", i.name = r, i.value = "", i.setAttribute(b, "true"), t.insertAdjacentElement("afterend", i), i;
}
function N(e) {
  if (Array.isArray(e))
    return null;
  ...
  if (!g(e))
    return null;
  const r = I(e.assetId ?? e.id ?? e.value), n = c(e.filename ?? e.title ?? e.label ?? e.name), i = c(e.url) || null;
  return !r && !n ? null : {
    assetId: r,
    filename: n,
    url: i
...
function ne(e) {
  V(e, w, (t) => {
    t.addValidator("fileLimit", ({ input: r }) => {
    ...
    }, ({ input: r, t: n }) => r.getAttribute("data-formie-validation-max-files-message") ?? n("Choose up to {files} files.", {
    ...
    }, ({ input: r, t: n }) => r.getAttribute("data-formie-validation-min-file-size-message") ?? n("File must be larger than {filesize} MB.", {
      ...
      return r.type !== "file" || !i || !("files" in r) || !r.files?.length ? !0 : Array.from(r.files).every((s) => s.size <= i);
    }, ({ input: r, t: n }) => r.getAttribute("data-formie-validation-max-file-size-message") ?? n("File must be smaller than {filesize} MB.", {
...
function le(e, t) {
  ...
  if (!g(r))
    return [];
  const n = c(r.fieldHandle), i = c(r.inputKey), s = c(r.inputName ?? r.name), o = g(r.data) ? r.data : null, l = c(o?.fieldHandle), a = c(o?.inputKey), p = c(o?.inputName ?? o?.name), d = [];
  let m = !1, u = !1, h = !1;
  if (i !== "" && i === t.inputKey && (m = !0, d.push(r.assets, r.assetIds, r.data)), s !== "" && s === t.inputName && (u = !0, d.push(r.assets, r.assetIds, r.data)), n === t.fieldHandle && (h = !0, d.push(r.assets, r.assetIds, r.data)), o) {
    const f = [
      o.assets,
      o.assetIds,
      o.uploadedAssets,
    ...
    a !== "" && a === t.inputKey && (m = !0, d.push(...f)), p !== "" && p === t.inputName && (u = !0, d.push(...f)), l === t.fieldHandle && (h = !0, d.push(...f));
  }
  [r.assets, r.assetIds, r.data].forEach((f) => {
    g(f) && (t.inputKey && t.inputKey in f && (m = !0, d.push(f[t.inputKey])), t.inputName in f && (u = !0, d.push(f[t.inputName])), t.fieldHandle in f && d.push(f[t.fieldHandle]));
  });
  for (const f of d) {
    const A = v(f);
    if (A.length && (m || u || h || re(A, t.pendingFiles)))
      return A;
  }
  return [];
}
  ...
  const n = {
    ...
    try {
      ...
      if (m !== n.hydrationToken)
        return;
      n.uploadedAssets = u, y(n), O(e, E, "uploaded-assets-sync", {
        fileUpload: e,
        assets: u
      });
    } catch (u) {
