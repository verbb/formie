import { a as e, r as t } from "./api-BBbsV96E.js";
import { r as n } from "./scripts-C4QAqZ0U.js";
//#region src/js/modules/captchas/recaptcha-shared.ts
async function r(r, i = !1, a) {
	let o = typeof r.language == "string" && r.language.trim() !== "" ? r.language.trim() : "en", { async: s, defer: c } = t(r.loadingMethod), l = i ? "https://www.google.com/recaptcha/enterprise.js" : "https://www.recaptcha.net/recaptcha/api.js", u = typeof a == "string" && a.trim() !== "" ? a.trim() : "explicit", d = new URL(l);
	return d.searchParams.set("render", u), d.searchParams.set("hl", o), u !== "explicit" && typeof r.badge == "string" && r.badge.trim() !== "" && d.searchParams.set("badge", r.badge.trim()), n("grecaptcha", {
		id: i ? "FORMIE_RECAPTCHA_ENTERPRISE_SCRIPT" : "FORMIE_RECAPTCHA_SCRIPT",
		src: d.toString(),
		async: s,
		defer: c,
		timeoutMs: e
	});
}
//#endregion
export { r as t };
