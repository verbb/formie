...
import { c as t, n, s as r, t as i } from "./shared-ktsx_SHX.js";
//#region src/js/modules/fields/checkbox-radio.ts
var a = "[data-formie-checkboxes-field-layout], [data-formie-radio-field-layout]", o = "minmaxOptions", s = "data-formie-checkbox-radio-max-disabled", c = "checkbox-radio", l = "checkbox-radio", u = e("fields", "checkbox-radio");
function d(e) {
...
function p(e) {
	t(e, l, (e) => {
		e.addValidator(o, ({ field: e, getRule: t }) => {
			if (!e || !t(o)) return !0;
			let n = Array.from(e.querySelectorAll("input[type=\"checkbox\"]")).filter((e) => e instanceof HTMLInputElement && !d(e)).filter((e) => e.checked).length, { min: r, max: i } = f(t);
			return !(r !== null && n < r || i !== null && n > i);
		}, ({ field: e, label: t, t: n, getRule: r }) => {
			let { min: i, max: a } = e ? f(r) : {
				min: null,
				max: null
			};
			return i !== null && a !== null ? n("{attribute} must select between {min} and {max}.", {
				attribute: t,
				min: i,
				max: a
			}) : i === null ? a === null ? n("{attribute} has an invalid value.", { attribute: t }) : n("{attribute} must select no greater than {max}.", {
				attribute: t,
				max: a
			}) : n("{attribute} must select no less than {min}.", {
				attribute: t,
				min: i
			});
		});
...
function _(e) {
	...
	if (!(t > 0)) return;
	let n = Array.from(e.querySelectorAll("input[type=\"checkbox\"]")).filter((e) => e instanceof HTMLInputElement && !d(e)), r = n.filter((e) => e.checked).length >= t;
	n.forEach((e) => {
...
function v(e, t) {
	Array.from(e.querySelectorAll("input[type=\"checkbox\"]")).filter((e) => e instanceof HTMLInputElement && e !== t && !d(e)).forEach((e) => {
		e.disabled && !e.checked || (e.checked = t.checked, h(e), e.dispatchEvent(new Event("change", { bubbles: !0 })), e.dispatchEvent(new Event("input", { bubbles: !0 })));
	});
}
function y(e, t) {
	if (!e.checked || !e.name) {
	...
	Array.from(t.querySelectorAll(`input[type="radio"][name="${n(e.name)}"]`)).filter((e) => e instanceof HTMLInputElement).forEach((e) => {
		h(e);
	});
}
function b(e) {
	...
	let n = t.map((t) => {
		let n = t.type === "radio" ? "change" : "click", r = () => {
			h(t), t.type === "checkbox" && d(t) && v(e, t), t.type === "radio" && y(t, e), g(e), _(e), u.log("Input interaction processed.", {
				inputName: t.name,
				inputType: t.type,
				checked: t.checked
			});
		};
		return t.addEventListener(n, r), h(t), () => {
			t.removeEventListener(n, r);
		};
	});
	return g(e), _(e), i(e, c, "init", { checkboxRadio: e }), () => {
		n.forEach((e) => {
			e();
		});
	};
...
var x = {
	...
	setup: async (e) => {
		...
		p(e.form), u.log("Module setup.", { fieldCount: t.length });
		let n = t.map((e) => b(e));
		return await e.emit("formie:module:checkbox-radio:init", { count: t.length }), { destroy: () => {
			n.forEach((e) => {
				e();
			}), m(e.form), u.log("Module destroy.", { fieldCount: t.length }), e.emit("formie:module:checkbox-radio:destroy", {});
		} };
	}
};
//#endregion
export { x as checkboxRadioModule };
