...
function m(e) {
	n(e, l, (e) => {
		e.addValidator(o, ({ field: e, getRule: t }) => {
			if (!e || !t(o)) return !0;
			let n = p(e), { min: r, max: i } = f(t);
			return !(r !== null && n < r || i !== null && n > i);
		}, ({ field: e, label: t, t: n, getRule: r }) => {
			if (!e) return n("{label} has an invalid value.", { label: t });
			let i = p(e), { min: a, max: o } = f(r);
			return a !== null && i < a ? e.getAttribute("data-formie-validation-min-options-message") ?? n("{label} should contain at least {min, number} {min, plural, one{option} other{options}}.", {
				...
				label: t,
				max: o
			}) : n("{label} has an invalid value.", { label: t });
		});
	});
}
function h(e) {
...
function y(e, t) {
	Array.from(e.querySelectorAll("input[type=\"checkbox\"]")).filter((e) => e instanceof HTMLInputElement && e !== t && !d(e)).forEach((e) => {
		e.disabled && !e.checked || (e.checked = t.checked, g(e), e.dispatchEvent(new Event("change", { bubbles: !0 })), e.dispatchEvent(new Event("input", { bubbles: !0 })));
	});
}
function b(e, t) {
	if (!e.checked || !e.name) {
...
function x(e) {
	...
	let n = t.map((t) => {
		let n = t.type === "radio" ? "change" : "click", r = () => {
			g(t), t.type === "checkbox" && d(t) && y(e, t), t.type === "radio" && b(t, e), _(e), v(e), u.log("Input interaction processed.", {
				inputName: t.name,
				inputType: t.type,
				checked: t.checked
			});
		};
		return t.addEventListener(n, r), g(t), () => {
			t.removeEventListener(n, r);
		};
	});
	return _(e), v(e), i(e, c, "init", { checkboxRadio: e }), () => {
		n.forEach((e) => {
			e();
		});
	};
...
var S = {
	...
	setup: async (e) => {
		...
		m(e.form), u.log("Module setup.", { fieldCount: t.length });
		let n = t.map((e) => x(e));
		return await e.emit("formie:module:checkbox-radio:init", { count: t.length }), { destroy: () => {
			n.forEach((e) => {
				e();
			}), h(e.form), u.log("Module destroy.", { fieldCount: t.length }), e.emit("formie:module:checkbox-radio:destroy", {});
		} };
	}
};
//#endregion
export { S as checkboxRadioModule };
