...
var b, x;
typeof navigator < "u" && typeof navigator.userAgent == "string" && (b = navigator, x = b.userAgent.toLowerCase());
function S() {
	return x.indexOf("headless") !== -1 || b.appVersion.indexOf("Headless") !== -1 || x.indexOf("bot") !== -1 || x.indexOf("crawl") !== -1 || b.webdriver === !0 || !b.language || b.languages !== void 0 && !b.languages.length;
}
function C(e) {
...
async function k(e, t, n) {
	...
	for (let e = 0; e < r.length; e++) try {
		...
		if (n.ok) return (await n.json()).data.puzzle;
		{
			...
			if (t && t.errors && t.errors[0] === "endpoint_not_enabled") throw Error(`Endpoint not allowed (${n.status})`);
...
var $ = class {
	...
	init() {
		...
		let t = new Blob([Z], { type: "text/javascript" });
		for (let n = 0; n < this.workers.length; n++) this.workers[n] = new Worker(Q.createObjectURL(t)), this.workers[n].onerror = (e) => this.errorCallback(e), this.workers[n].onmessage = (t) => {
			let r = t.data;
			if (r) if (r.type === "ready") this.readyCount++, this.solverType = r.solver, this.readyCount == this.workers.length && (e(), this.readyCallback());
				...
				if (this.puzzleIndex < this.puzzleSolverInputs.length && (this.workers[n].postMessage({
					...
					this.doneCallback({
						...
						h: this.totalHashes,
						t: e,
						diagnostics: X(this.solverType, e),
	...
	async start(e) {
		await this.readyPromise, this.puzzleSolverInputs = D(e.buffer, e.n), this.solutionBuffer = new Uint8Array(8 * e.n), this.numPuzzles = e.n, this.threshold = e.threshold, this.puzzleIndex = 0, this.puzzleNumber++;
...
}, te = "https://api.friendlycaptcha.com/api/v1/puzzle", ne = class {
	constructor(e, t = {}) {
		this.workerGroup = new $(), this.valid = !1, this.needsReInit = !1, this.hasBeenStarted = !1, this.hasBeenDestroyed = !1, this.opts = Object.assign({
			forceJSFallback: !1,
			skipStyleInjection: !1,
			startMode: "focus",
			puzzleEndpoint: e.dataset.puzzleEndpoint || te,
			startedCallback: () => 0,
			readyCallback: () => 0,
			doneCallback: () => 0,
			errorCallback: () => 0,
			...
			styleNonce: null
		}, t), this.e = e, this.e.friendlyChallengeWidget = this, this.loadLanguage(), e.innerText = this.lang.text_init, this.opts.skipStyleInjection || g(this.opts.styleNonce), this.init(this.opts.startMode === "auto" || this.e.dataset.start === "auto");
	}
	init(e) {
		...
		if (this.initWorkerGroup(), e) this.start();
			...
			let e = v(this.e);
			e ? y(e, () => this.start()) : console.log("FriendlyCaptcha div seems not to be contained in a form, autostart will not work");
		}
	}
	loadLanguage(e) {
		if (e === void 0 ? this.e.dataset.lang && (this.opts.language = this.e.dataset.lang) : this.opts.language = e, typeof this.opts.language == "string") {
			let e = this.opts.language.toLowerCase(), t = Y[e];
			t === void 0 && e[2] === "-" && (e = e.substring(0, 2), t = Y[e]), t === void 0 && (console.error("FriendlyCaptcha: language \"" + this.opts.language + "\" not found."), t = Y.en), this.lang = t;
		} else this.lang = Object.assign(Object.assign({}, Y.en), this.opts.language);
	}
	makeButtonStart() {
	...
	onWorkerError(e) {
		this.hasBeenStarted = !1, this.needsReInit = !0, this.expiryTimeout && clearTimeout(this.expiryTimeout), console.error("[FRC]", e), this.e.innerHTML = h(this.opts.solutionFieldName, this.lang, "Background worker error " + e.message), this.makeButtonStart(), this.opts.forceJSFallback = !0;
	}
	initWorkerGroup() {
		this.workerGroup.progressCallback = (e) => {
			_(this.e, e);
		}, this.workerGroup.readyCallback = () => {
			this.e.innerHTML = u(this.opts.solutionFieldName, this.lang), this.makeButtonStart(), this.opts.readyCallback();
		}, this.workerGroup.startedCallback = () => {
			this.e.innerHTML = f(this.opts.solutionFieldName, this.lang), this.opts.startedCallback();
	...
	expire() {
		this.hasBeenStarted = !1, this.e.isConnected !== !1 && (this.e.innerHTML = m(this.opts.solutionFieldName, this.lang), this.makeButtonStart());
	}
	async start() {
		if (this.hasBeenDestroyed) {
		...
		if (!e) {
			console.error("FriendlyCaptcha: sitekey not set on frc-captcha element"), this.e.innerHTML = h(this.opts.solutionFieldName, this.lang, "Website problem: sitekey not set", !1);
			return;
		}
		if (S()) {
			this.e.innerHTML = h(this.opts.solutionFieldName, this.lang, "Browser check failed, try a different browser", !1, !0);
			return;
		}
		if (this.needsReInit) {
		...
		try {
			this.e.innerHTML = d(this.opts.solutionFieldName, this.lang), this.puzzle = O(await k(this.opts.puzzleEndpoint, e, this.lang)), this.expiryTimeout && clearTimeout(this.expiryTimeout), this.expiryTimeout = setTimeout(() => this.expire(), this.puzzle.expiry - 3e4);
		} catch (e) {
			console.error("[FRC]", e), this.hasBeenStarted = !1, this.expiryTimeout && clearTimeout(this.expiryTimeout), this.e.innerHTML = h(this.opts.solutionFieldName, this.lang, e.message), this.makeButtonStart(), this.opts.errorCallback({
	...
	handleDone(e) {
		...
		return this.e.innerHTML = p(this.opts.solutionFieldName, this.lang, t, e), this.needsReInit = !0, t;
	}
	destroy() {
...
//#endregion
export { ne as WidgetInstance, Y as localizations };
