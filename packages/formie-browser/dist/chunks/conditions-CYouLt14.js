import { h as j, g as U } from "./index-DtUMIYEa.js";
import { e as R } from "./shared-8Shdc9Qt.js";
import { j as _, h as W } from "./index-MuyEvWaf.js";
...
function ae(e) {
  return Array.from(e.querySelectorAll(v)).filter((t) => p(t));
}
function ue(e) {
...
function P(e, t, r) {
  ...
  const l = _(n.handle), c = R(l), m = Array.from(e.querySelectorAll(`[name="${c}"]`)).filter((f) => p(f)), S = Array.from(e.querySelectorAll(`[name="${c}[]"]`)).filter((f) => p(f));
  ...
  const h = se(t);
  if (h) {
    const f = _(n.handle.replace(/__ROW__/g, h)), b = R(f), s = Array.from(e.querySelectorAll(`[name="${b}"]`)).filter((u) => p(u)), a = Array.from(e.querySelectorAll(`[name="${b}[]"]`)).filter((u) => p(u));
    if (s.length || a.length)
      return [...s, ...a];
  }
  const N = _(n.handle).replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/__ROW__/g, "\\d+"), E = new RegExp(N);
  return Array.from(e.querySelectorAll("[name]")).filter((f) => p(f) && E.test(f.getAttribute("name") || ""));
}
function fe(e) {
  ...
  switch (r) {
    ...
    case "format": {
      ...
      if (Number.isFinite(i) && e.trim() !== "")
        return he(i, n);
      const o = n.preset || "", l = o === "custom" ? n.pattern || "" : pe(o);
      if (!l)
        return e;
      const c = new Date(e);
      return Number.isNaN(c.getTime()) ? e : ge(c, l);
    }
    case "lower":
  ...
  setup: async (e) => {
    ...
    const l = () => {
      ...
      return n.forEach((a) => {
        ...
        s = s || d, g.log("Condition evaluated.", {
      ...
      for (let s = 0; s < M && m(); s += 1)
        s === M - 1 && g.warn("Reached max evaluation passes.", { maxPasses: M });
    }, h = () => {
      ...
      l(), n = c(), N(), g.log("Rebuilt condition graph.", {
        entryCount: n.length
      }), h();
    }, f = () => {
    ...
    return b.observe(t, {
      ...
      destroy: () => {
        l(), b.disconnect(), g.log("Module destroy."), e.emit("formie:module:conditions:destroy", {});
      }
    };
  }
};
