...
${e === "-" ? "" : `<input name="${e}" class="frc-captcha-solution" type="hidden" value="${a}">`}`;
}
function ee(e, t) {
	return l(e, t.rtl, "<path d=\"M17,11c0.34,0,0.67,0.04,1,0.09V6.27L10.5,3L3,6.27v4.91c0,4.54,3.2,8.79,7.5,9.82c0.55-0.13,1.08-0.32,1.6-0.55 C11.41,19.47,11,18.28,11,17C11,13.69,13.69,11,17,11z\"/><path d=\"M17,13c-2.21,0-4,1.79-4,4c0,2.21,1.79,4,4,4s4-1.79,4-4C21,14.79,19.21,13,17,13z M17,14.38\"/>", !0, t.text_ready, ".UNSTARTED", t.button_start, !1);
}
function u(e, t) {
	return l(e, t.rtl, s, !0, t.text_fetching, ".FETCHING", void 0, !0);
}
function d(e, t) {
	return l(e, t.rtl, s, !0, t.text_solving, ".UNFINISHED", void 0, !0);
}
function f(e, t, n, r) {
	let i = `${r.t.toFixed(0)}s (${(r.h / r.t * .001).toFixed(0)}K/s)${r.solver === a ? " JS Fallback" : ""}`;
	return l(e, t.rtl, `<title>${t.text_completed_sr}</title><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm-2 16l-4-4 1.41-1.41L10 14.17l6.59-6.59L18 9l-8 8z"></path>`, !1, t.text_completed, n, void 0, !1, i, "frc-success");
}
function p(e, t) {
	return l(e, t.rtl, c, !0, t.text_expired, ".EXPIRED", t.button_restart);
}
function m(e, t, n, r = !0, i = !1) {
	return l(e, t.rtl, c, !0, `<b>${t.text_error}</b><br>${n}`, i ? ".HEADLESS_ERROR" : ".ERROR", r ? t.button_retry : void 0);
}
function h(e = null) {
	if (!document.querySelector("#frc-style")) {
		let t = document.createElement("style");
		t.id = "frc-style", t.innerHTML = i, e && t.setAttribute("nonce", e), document.head.appendChild(t);
	}
}
function g(e, t) {
	let n = e.querySelector(".frc-progress"), r = (t.i + 1) / t.n;
	n && (n.value = r, n.innerText = (r * 100).toFixed(1) + "%", n.title = t.i + 1 + "/" + t.n + " (" + (t.h / t.t * .001).toFixed(0) + "K/s)");
}
function _(e) {
	for (; e.tagName !== "FORM";) if (e = e.parentElement, !e) return null;
	return e;
}
function v(e, t) {
...
var y, b;
typeof navigator < "u" && typeof navigator.userAgent == "string" && (y = navigator, b = y.userAgent.toLowerCase());
function x() {
	return b.indexOf("headless") !== -1 || y.appVersion.indexOf("Headless") !== -1 || b.indexOf("bot") !== -1 || b.indexOf("crawl") !== -1 || y.webdriver === !0 || !y.language || y.languages !== void 0 && !y.languages.length;
}
function S(e) {
	return e > 255 ? e = 255 : e < 0 && (e = 0), 2 ** ((255.999 - e) / 8) >>> 0;
}
var C = 13, w = 14, T = 15;
function E(e, t) {
	let n = [];
	for (let r = 0; r < t; r++) {
		...
		t.set(e), t[120] = r, n.push(t);
	}
	return n;
}
var D = class extends Error {
	constructor(e) {
		super(e), this.name = "UserFacingError", this.rawError = this;
	}
};
function O(e) {
	let t = e.split("."), n = t[1], i = r(n);
	return {
		signature: t[0],
		base64: n,
		buffer: i,
...
async function k(e, t, n) {
	...
	for (let e = 0; e < r.length; e++) try {
		let n = await A(r[e] + "?sitekey=" + t, {
			headers: [["x-frc-client", "js-0.9.20"]],
			mode: "cors"
		}, 2);
		if (n.ok) return (await n.json()).data.puzzle;
		{
			let t;
			try {
				t = await n.json();
			} catch {}
			if (t && t.errors && t.errors[0] === "endpoint_not_enabled") throw new D(`Endpoint not allowed (${n.status})`);
			if (t && t.errors && t.errors[0] === "account_usage_limit_reached") throw new D("Usage limit reached");
			if (e === r.length - 1) throw Error(`Response status ${n.status} ${n.statusText} ${t ? t.errors : ""}`);
		}
	} catch (t) {
		if (t instanceof D) throw t;
		console.error("[FRC Fetch]:", t);
		let i = /* @__PURE__ */ Error(`${n.text_fetch_error} <a class="frc-err-url" href="${r[e]}">${r[e]}</a>`);
		throw i.rawError = t, i;
	}
	throw Error("Internal error");
}
async function A(e, t, n) {
	let r = 1e3;
	return fetch(e, t).catch(async (i) => {
		if (n === 0) throw i;
		return await new Promise((e) => setTimeout(e, r)), r *= 4, A(e, t, n - 1);
	});
}
var j = {
...
var te = class {
			...
			if (r) {
				if (r.type === "ready") this.readyCount++, this.solverType = r.solver, this.readyCount == this.workers.length && (e(), this.readyCallback());
					...
					if (this.puzzleIndex < this.puzzleSolverInputs.length && (this.workers[n].postMessage({
						...
						i: this.progress
					}), this.solutionBuffer.set(r.solution, r.puzzleIndex * 8), this.progress == this.numPuzzles) {
						let e = (Date.now() - this.startTime) / 1e3;
						this.doneCallback({
							solution: this.solutionBuffer,
							h: this.totalHashes,
							t: e,
							diagnostics: Z(this.solverType, e),
							solver: this.solverType
						});
					}
				} else r.type === "error" && this.errorCallback(r);
			}
		};
	}
	setupSolver(e = !1) {
		...
		for (let e = 0; e < this.workers.length; e++) this.workers[e].postMessage(t);
	}
	async start(e) {
		await this.readyPromise, this.puzzleSolverInputs = E(e.buffer, e.n), this.solutionBuffer = new Uint8Array(8 * e.n), this.numPuzzles = e.n, this.threshold = e.threshold, this.puzzleIndex = 0, this.puzzleNumber++;
	...
	terminateWorkers() {
		if (this.workers.length != 0) {
			for (let e = 0; e < this.workers.length; e++) this.workers[e].terminate();
			this.workers = [];
		}
	}
}, ne = "https://api.friendlycaptcha.com/api/v1/puzzle", re = class {
	constructor(e, t = {}) {
		this.workerGroup = new te(), this.valid = !1, this.needsReInit = !1, this.hasBeenStarted = !1, this.hasBeenDestroyed = !1, this.opts = Object.assign({
			forceJSFallback: !1,
			skipStyleInjection: !1,
			startMode: "focus",
			puzzleEndpoint: e.dataset.puzzleEndpoint || ne,
			startedCallback: () => 0,
			readyCallback: () => 0,
			doneCallback: () => 0,
			errorCallback: () => 0,
			...
			styleNonce: null
		}, t), this.e = e, this.e.friendlyChallengeWidget = this, this.loadLanguage(), e.innerText = this.lang.text_init, this.opts.skipStyleInjection || h(this.opts.styleNonce), this.init(this.opts.startMode === "auto" || this.e.dataset.start === "auto");
	}
	init(e) {
		...
		if (this.initWorkerGroup(), e) this.start();
		else if (this.e.dataset.start !== "none" && (this.opts.startMode === "focus" || this.e.dataset.start === "focus")) {
			let e = _(this.e);
			e ? v(e, () => this.start()) : console.log("FriendlyCaptcha div seems not to be contained in a form, autostart will not work");
		}
	}
	loadLanguage(e) {
		if (e === void 0 ? this.e.dataset.lang && (this.opts.language = this.e.dataset.lang) : this.opts.language = e, typeof this.opts.language == "string") {
			let e = this.opts.language.toLowerCase(), t = X[e];
			t === void 0 && e[2] === "-" && (e = e.substring(0, 2), t = X[e]), t === void 0 && (console.error("FriendlyCaptcha: language \"" + this.opts.language + "\" not found."), t = X.en), this.lang = t;
		} else this.lang = Object.assign(Object.assign({}, X.en), this.opts.language);
	}
	makeButtonStart() {
		let e = this.e.querySelector("button");
		e && (e.addEventListener("click", (e) => this.start(), {
			once: !0,
			passive: !0
		}), e.addEventListener("touchstart", (e) => this.start(), {
			once: !0,
			passive: !0
		}));
	}
	onWorkerError(e) {
		this.hasBeenStarted = !1, this.needsReInit = !0, this.expiryTimeout && clearTimeout(this.expiryTimeout), console.error("[FRC]", e), this.e.innerHTML = m(this.opts.solutionFieldName, this.lang, "Background worker error " + e.message), this.makeButtonStart(), this.opts.forceJSFallback = !0;
	}
	initWorkerGroup() {
		this.workerGroup.progressCallback = (e) => {
			g(this.e, e);
		}, this.workerGroup.readyCallback = () => {
			this.e.innerHTML = ee(this.opts.solutionFieldName, this.lang), this.makeButtonStart(), this.opts.readyCallback();
		}, this.workerGroup.startedCallback = () => {
			this.e.innerHTML = d(this.opts.solutionFieldName, this.lang), this.opts.startedCallback();
		}, this.workerGroup.doneCallback = (e) => {
			let t = this.handleDone(e);
			this.opts.doneCallback(t);
			let n = this.e.dataset.callback;
			n && window[n](t);
	...
	expire() {
		this.hasBeenStarted = !1, this.e.isConnected !== !1 && (this.e.innerHTML = p(this.opts.solutionFieldName, this.lang), this.makeButtonStart());
	}
	async start() {
		if (this.hasBeenDestroyed) {
		...
		let e = this.opts.sitekey || this.e.dataset.sitekey;
		if (!e) {
			console.error("FriendlyCaptcha: sitekey not set on frc-captcha element"), this.e.innerHTML = m(this.opts.solutionFieldName, this.lang, "Website problem: sitekey not set", !1);
			return;
		}
		if (x()) {
			this.e.innerHTML = m(this.opts.solutionFieldName, this.lang, "Browser check failed, try a different browser", !1, !0);
			return;
		}
		if (this.needsReInit) {
		...
		try {
			this.e.innerHTML = u(this.opts.solutionFieldName, this.lang), this.puzzle = O(await k(this.opts.puzzleEndpoint, e, this.lang)), this.expiryTimeout && clearTimeout(this.expiryTimeout), this.expiryTimeout = setTimeout(() => this.expire(), this.puzzle.expiry - 3e4);
		} catch (e) {
			console.error("[FRC]", e), this.hasBeenStarted = !1, this.expiryTimeout && clearTimeout(this.expiryTimeout), this.e.innerHTML = m(this.opts.solutionFieldName, this.lang, e.message), this.makeButtonStart(), this.opts.errorCallback({
				code: "error_getting_puzzle",
				description: e.toString(),
				error: e
			});
			let t = this.e.dataset["callback-error"];
			t && window[t](this);
			return;
		}
	...
	handleDone(e) {
		this.valid = !0;
		let t = `${this.puzzle.signature}.${this.puzzle.base64}.${n(e.solution)}.${n(e.diagnostics)}`;
		return this.e.innerHTML = f(this.opts.solutionFieldName, this.lang, t, e), this.needsReInit = !0, t;
	}
	destroy() {
...
//#endregion
export { re as WidgetInstance, X as localizations };
