...
var o = e({
	...
	load: async (e) => {
		...
		return i?.trim() ? (await t("paypal", {
			id: r,
			src: a(i, n.currency || "AUD")
		}), window.paypal) : (console.error("[formie] Missing clientId for PayPal."), null);
	},
	mount: async (e) => {
		let { api: t, field: n, services: r, options: a, provider: o } = e, s = n.querySelector("[data-formie-paypal-button]");
		if (!s || !t || s.getAttribute("data-formie-paypal-rendered") === "true") return null;
		s.innerHTML = "";
		let c = !!o.useSandbox, l = r.resolveCurrency({
			value: o.currency,
			defaultCurrency: "AUD"
		});
		if (!l.ok) return r.addError("error" in l ? l.error : "Invalid PayPal currency."), null;
		let u = l.value, d = {
		...
		d.layout === "horizontal" && (d.tagline = o.buttonTagline ?? !0);
		let f = {
			env: c ? "sandbox" : "production",
			style: d,
			createOrder: (e, t) => {
				r.removeError();
			...
			onApprove: async (e, t) => {
				try {
					...
					r.updateInputs("paypalOrderId", e.orderID), r.updateInputs("paypalAuthId", n || ""), n ? r.addSuccess("Payment authorized. Finalize the form to complete payment.") : r.addSuccess("PayPal approval received. Finalizing payment on submit.");
				} catch {
					r.addError("Unable to authorize payment. Please try again.");
				}
			}
		}, p = t.Buttons(f);
		return s.setAttribute("data-formie-paypal-rendered", "true"), p.render(s);
	},
	unmount: async (e) => {
	...
	onAfterSubmit: async ({ services: e }) => {
		e.updateInputs(["paypalOrderId", "paypalAuthId"], ""), e.removeSuccess(), e.removeError();
	}
});
//#endregion
export { o as paypalModule };
