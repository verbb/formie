...
type MonerisProviderOptions = {
    endpointUrl?: string;
};

type MonerisTokenResult = {
    token: string;
    error: string;
};

type MonerisFieldState = Element & {
    __formieMonerisAuthorizeCleanup?: (() => void) | null;
    __formieMonerisAuthorizeRequestId?: number;
};

const MONERIS_RESPONSE_MESSAGES: Record<string, string> = {
...
const extractMonerisToken = (payload: unknown): MonerisTokenResult => {
    const fromObject = (value: Record<string, unknown>): MonerisTokenResult => {
        ...
        if (responseCode && responseCode !== '001') {
            ...
            return { token: '', error: responseMessage || codeMessage };
        }
    ...
    try {
        ...
        if (decoded && typeof decoded === 'object') {
            return fromObject(decoded as Record<string, unknown>);
        }
    } catch {
        // Continue parsing other known payload shapes.
    }
    ...
    // Moneris can return a raw token string. Accept only token-like values.
...
export const monerisModule = definePaymentModule<MonerisProviderOptions, null, null>({
    ...
    onBeforeAuthorize: async(args) => {
        ...
        const iframe = field.querySelector<HTMLIFrameElement>('[data-formie-moneris-frame]');
        let endpointOrigin = '';
        ...
        // Defensive cleanup if an earlier authorize attempt left handlers behind.
        ...
        return new Promise<boolean>((resolve) => {
            ...
            const cleanup = () => {
                ...
                if (fieldState.__formieMonerisAuthorizeCleanup === cleanup) {
                    fieldState.__formieMonerisAuthorizeCleanup = null;
                }
            };

            const messageHandler = (event: MessageEvent) => {
                if (resolved || requestId !== fieldState.__formieMonerisAuthorizeRequestId) {
                    return;
                }
    ...
    setup: async(ctx) => {
        ...
        const messageHandler = (event: MessageEvent) => {
            if (!event.origin.includes('moneris')) {
                return;
            }
        ...
        const unbind = services.events.onRoot('message', messageHandler as EventListener);

        return {
            destroy: () => {
                unbind();
                const fieldState = ctx.target as MonerisFieldState;
                fieldState.__formieMonerisAuthorizeCleanup?.();
                fieldState.__formieMonerisAuthorizeCleanup = null;
            },
        };
